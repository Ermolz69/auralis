...
import type { SubtitleTrack } from '../src/shared/api/contracts/subtitle';
...
const storyTranscripts: Record<string, Transcript> = {
  'workspace-story': {
    language: 'ru',
    segments: [
      { id: 'ru-1', index: 0, startMs: 1200, endMs: 7450, sourceText: 'Добро пожаловать в подробный разбор нейросетевого синтеза звука.' },
      { id: 'ru-2', index: 1, startMs: 8100, endMs: 12880, sourceText: 'Мы рассмотрим современные системы преобразования текста в речь.' },
      { id: 'ru-3', index: 2, startMs: 13200, endMs: 18560, sourceText: 'Начнём с основ акустического моделирования.' },
    ],
  },
  'youtube-ready': {
    language: 'en',
...
export function installTauriStoryAdapter() {
  mockIPC(async (cmd, payload) => {
    ...
    switch (cmd) {
      ...
      case 'list_youtube_subtitle_tracks_cmd':
...
function handleListYoutubeSubtitleTracks(): SubtitleTrack[] {
  return [
    { id: 'ru-auto-vtt', language: 'ru', label: 'Russian', format: 'vtt', isAutoGenerated: true },
    { id: 'en-manual-vtt', language: 'en', label: 'English', format: 'vtt', isAutoGenerated: false },
    { id: 'en-auto-vtt', language: 'en', label: 'English', format: 'vtt', isAutoGenerated: true },
  ];
}
