...
describe('RunDubbing', () => {
  ...
  const createMockContext = (overrides = {}) => {
    const ctx = {
      projectId: mockProject.id, project: mockProject, setProjectId: vi.fn(), setProject: vi.fn(),
      deletingProjectId: null as string | null, beginProjectDeletion: vi.fn(), finishProjectDeletion: vi.fn(), operationGeneration: 1,
      captureToken: () => ({ generation: ctx.operationGeneration }),
      validateToken: (t: any) => ctx.deletingProjectId === null && t.generation === ctx.operationGeneration,
      ...overrides,
    };
    return ctx;
  };
  ...
  it('is disabled during project deletion', () => {
    ...
    render(<ProjectContext.Provider value={ctx}><RunDubbing /></ProjectContext.Provider>);
    expect((screen.getByRole('button', { name: /run dubbing/i }) as HTMLButtonElement).disabled).toBe(true);
  });

  it('is enabled when project is eligible and not deleting', () => {
    const ctx = createMockContext();
    render(<ProjectContext.Provider value={ctx}><RunDubbing /></ProjectContext.Provider>);
    expect((screen.getByRole('button', { name: /run dubbing/i }) as HTMLButtonElement).disabled).toBe(false);
  });

  it('is disabled when another project is deleting', () => {
    const ctx = createMockContext({ deletingProjectId: 'other-id' });
    const { rerender } = render(<ProjectContext.Provider value={ctx}><RunDubbing /></ProjectContext.Provider>);
    expect((screen.getByRole('button', { name: /run dubbing/i }) as HTMLButtonElement).disabled).toBe(true);
    ctx.deletingProjectId = null;
    rerender(<ProjectContext.Provider value={ctx}><RunDubbing /></ProjectContext.Provider>);
    expect((screen.getByRole('button', { name: /run dubbing/i }) as HTMLButtonElement).disabled).toBe(false);
  });
  ...
  it('verifies context switch resets loading for the new RunDubbing project', async () => {
    ...
    expect(screen.getByRole('button', { name: /run dubbing/i }).textContent).not.toBe('Starting...');
    await act(async () => {
  ...
  it('fences out success when deletion occurs during RunDubbing', async () => {
    ...
    const ctx = createMockContext();
    const { rerender } = render(<ProjectContext.Provider value={ctx}><RunDubbing /></ProjectContext.Provider>);
    act(() => { screen.getByRole('button', { name: /run dubbing/i }).click(); });
    ctx.operationGeneration += 1;
