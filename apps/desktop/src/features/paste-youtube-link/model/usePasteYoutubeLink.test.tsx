...
vi.mock('@/entities/project', () => ({ createProjectFromYoutube: vi.fn(), useProjectContext: vi.fn() }));
...
describe('usePasteYoutubeLink', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockContextValue = {
      projectId: 'p-1', setProjectId: vi.fn(), setProject: vi.fn(), deletingProjectId: null,
      beginProjectDeletion: vi.fn(), finishProjectDeletion: vi.fn(), operationGeneration: 1,
      captureToken: () => ({ generation: mockContextValue.operationGeneration }),
      validateToken: (token: any) => mockContextValue.deletingProjectId === null && token.generation === mockContextValue.operationGeneration,
    };
  ...
  it('blocks startProject when deletingProjectId is active', async () => {
    ...
    act(() => { result.current.setUrl('https://youtube.com/watch?v=123'); });
    let res;
    await act(async () => { res = await result.current.startProject(); });
    expect(res).toBeNull();
    expect(createProjectFromYoutube).not.toHaveBeenCalled();
  });

  it('allows startProject when deletingProjectId is null', async () => {
    vi.mocked(createProjectFromYoutube).mockResolvedValue({ project: { id: 'p-new', title: 'New' } as any, job: {} as any });
    const { result } = renderHook(() => usePasteYoutubeLink());
  ...
  it('fences out response when delete occurs during create request', async () => {
    ...
    act(() => { startPromise = result.current.startProject(); });
    expect(result.current.isStarting).toBe(true);
    act(() => { mockContextValue.operationGeneration += 1; mockContextValue.deletingProjectId = 'p-1'; });
    rerender();
    await act(async () => {
  ...
  it('fences out response when switch occurs during create request', async () => {
    ...
    let startPromise: Promise<any>;
    act(() => { startPromise = result.current.startProject(); });
    act(() => { mockContextValue.projectId = 'p-other'; mockContextValue.operationGeneration += 1; });
    rerender();
    await act(async () => {
      resolveCreate({ project: { id: 'p-new' } as any, job: {} as any });
      await startPromise;
  ...
  it('stale YouTube NOT_FOUND error does not set the state/error', async () => {
    ...
    let startPromise: Promise<any>;
    act(() => { startPromise = result.current.startProject(); });
    act(() => { mockContextValue.operationGeneration += 1; });
    rerender();
    await act(async () => {
  ...
  it('blocks concurrent second request inside the same generation', async () => {
    ...
    act(() => { firstPromise = result.current.startProject(); });
    let secondRes: any;
    await act(async () => { secondRes = await result.current.startProject(); });
  ...
  it('allows a new request after generation transition and prevents old finally from resetting loading/lock', async () => {
    ...
    act(() => { startPromise1 = result.current.startProject(); });
    expect(result.current.isStarting).toBe(true);
    act(() => { mockContextValue.operationGeneration += 1; });
    rerender();
    ...
    act(() => { startPromise2 = result.current.startProject(); });
    expect(result.current.isStarting).toBe(true);
    await act(async () => {
  ...
  it('releases activeAttempt lock on actual error', async () => {
    ...
    const { result } = renderHook(() => usePasteYoutubeLink());
    act(() => { result.current.setUrl('https://youtube.com/watch?v=123'); });
    await act(async () => { await result.current.startProject(); });
    expect(result.current.error).toBe('Error: Failed');
    expect(result.current.isStarting).toBe(false);
    vi.mocked(createProjectFromYoutube).mockResolvedValue({ project: { id: 'p-new' } as any, job: {} as any });
    let res;
    await act(async () => { res = await result.current.startProject(); });
    expect(res).not.toBeNull();
  });
