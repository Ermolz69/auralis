...
export function StreamsTable({ streams }: StreamsTableProps) {
  if (!streams || streams.length === 0) {
    return <div className="text-sm text-muted-foreground p-3 bg-muted/20 rounded-md">No streams found</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      {streams.map((s) => (
        <div key={s.index} className="flex flex-col gap-1.5 p-3 bg-surface border border-muted rounded-lg shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-muted-foreground bg-muted/30 px-1.5 py-0.5 rounded">#{s.index}</span>
          </div>
          <div className="flex flex-col ml-8 text-xs text-muted-foreground">
            <span className="font-medium text-text">{s.codec_name?.toUpperCase()} {s.codec_long_name ? `(${s.codec_long_name})` : ''}</span>
