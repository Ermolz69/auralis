...
- **frontend quality**: Runs FSD boundary checks, color-token checks, the Storybook metadata and hierarchy contract, file-size and duplicate-code checks, bundle/CSP policy regressions, IPC contract parity, release metadata parity, and release smoke-tool tests. It is part of `task check:frontend`; use `task check:quality:frontend` for an isolated policy-only run.
...
- **native acceptance**: The manually triggered `Tauri Build` workflow runs the real React → IPC → Rust → SQLite → bundled `ffprobe`/`yt-dlp` → filesystem scenario on Windows (`task desktop:e2e:native`). Its local HTTP fixture, database, WebView2 profile, staging/workspace data, and frontend test build are removed on success or failure. It is not duplicated on every PR.
...
- **Storybook**: `task check:storybook` runs the catalog and default-deny control-safety contract, all browser stories and accessibility checks, the allowed-value render matrix, Windows screenshot regression, overflow checks, and a static build. Pull-request CI runs browser stories and the metadata contract through `task check:frontend`, then compares committed visual baselines in the pinned `windows-2025` job. Visual failures upload screenshots, traces, and reports from `apps/desktop/storybook-visual-results`; the dedicated Pages workflow owns the static build.
